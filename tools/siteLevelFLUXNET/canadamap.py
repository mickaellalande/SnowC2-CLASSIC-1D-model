...
def biome_plot(df):
    ...
    rp = ccrs.RotatedPole(pole_longitude=83.0,
                        pole_latitude=42.5,
    ...
    ax.set_extent([-130,-55,40,80],crs=ccrs.PlateCarree())

    for n,b in enumerate(biomedict):
        bio=df[df['biome'].str.match(b)]
        if b == 'SAV':
            bio.plot.scatter(x='longitude', y='latitude',s=10*bio['total_years'],ax=ax,label=b,color='tab:pink',
                    linewidth=1,  marker='o',alpha=0.9,edgecolors='black',
                    transform=ccrs.PlateCarree()
                    )
        elif b == 'WET':
            bio.plot.scatter(x='longitude', y='latitude',s=10*bio['total_years'],ax=ax,label=b,color='tab:olive',
                    linewidth=1,  marker='o',alpha=0.9,edgecolors='black',
                    transform=ccrs.PlateCarree()
                    )
        else:
            bio.plot.scatter(x='longitude', y='latitude',s=10*bio['total_years'],ax=ax,label=b,color=colors2[n],
                    linewidth=1,  marker='o',alpha=0.9,edgecolors='black',
                    transform=ccrs.PlateCarree()
                    )

    plt.text(-154,72,str(str(numsites) + " sites"),fontsize=22,transform=ccrs.PlateCarree())
    ...
    legend = ax.legend(handles=biomepatches, title="Biomes", prop = {'size':'x-large'},
            ...
            loc="upper right", ncol=1, frameon=True, fontsize=22)

    labels = ["2", "5", "12", "20"]
    l1 = ax.scatter([],[], s=20, edgecolors='none',facecolors='k')
    l2 = ax.scatter([],[], s=50, edgecolors='none',facecolors='k')
    l3 = ax.scatter([],[], s=120, edgecolors='none',facecolors='k')
    l4 = ax.scatter([],[], s=200, edgecolors='none',facecolors='k')
    leg = ax.legend([l1, l2, l3, l4], labels, ncol=4, frameon=True, fontsize=16,
    handlelength=0.5, loc = "lower left", borderpad = 0.5, 
    ...
    plt.savefig(str(outputFiles + 'mapofFLUXNETsites-canada-biome.png'),dpi=450, bbox_inches='tight')
    plt.show()
...
def landscape_plot(df):
    ...
    for n,b in enumerate(landscapedict):
        ...
        lscape.plot.scatter(x='longitude', y='latitude',s=10*lscape['total_years'],ax=ax,label=b,color=colors3[n],
                linewidth=1,  marker='o',alpha=0.9,edgecolors='black',
                transform=ccrs.PlateCarree()
                )
    ...
    plt.savefig(str(outputFiles + 'mapofFLUXNETsites-canada-landscapeStatus.png'),dpi=450, bbox_inches='tight')
    plt.show()
...
def peatland_plot(df):
    ...
    for n,b in enumerate(peatdict):
        ...
        ptlnd.plot.scatter(x='longitude', y='latitude',s=10*ptlnd['total_years'],ax=ax,label=b,color=colors3[n],
    ...
    plt.savefig(str(outputFiles + 'mapofFLUXNETsites-canada-peatland.png'),dpi=450, bbox_inches='tight')
    plt.show()
